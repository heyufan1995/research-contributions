...
class SwinunetrAlgo(BundleAlgo):
    def fill_template_config(self, data_stats_file: Optional[str] = None, output_path: Optional[str] = None, **kwargs):
        ...
        if kwargs.pop("fill_with_datastats", True):
            ...
            _, _, _, batch_size = auto_adjust_network_settings(
            ...
            config["roi_size"] = input_config.get("roi_size", [96,96,96])
            ...
            if any([r < 0.8 * i for r, i in zip(config["roi_size"], image_size)]):
